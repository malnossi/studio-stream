...
if [ -f "${DEPS_DIST}/lib/libmp3lame.a" ] && [ -f "${DEPS_DIST}/lib/libportaudio.a" ] && [ -f "${DEPS_DIST}/lib/libfdk-aac.a" ]; then
...
export CFLAGS="-Wno-error=date-time -Wno-error"
export CXXFLAGS="-Wno-error=date-time -Wno-error"
...
make -j$(nproc 2>/dev/null || echo 4) || true
make install || true
...
make lib/libportaudio.la -j$(nproc 2>/dev/null || echo 4) || true
make install || true
